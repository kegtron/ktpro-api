...
- **data**: Notification-specific data (optional) 
  - See the Device GET endpoint for device object definitions

# Node.js Example
```js
const Websocket = require('ws');  // npm install -g ws
const addr = 'wss://mdash.net/api/v2/m/device/notify?access_token=<DEVICE_PUBLIC_KEY>'; // Update device pubkey
